...
const SITE_URL =
  ...
  'https://nextrep-fd6a57ecb582.herokuapp.com/';

// NextRep brand palette (from public/nextrep.avif)
const BRAND_DARK = '#232222'; // near-black mark color; used for active UI on white
const BRAND_LIME = '#BEFD6E'; // lime accent
...
async function registerForPushNotifications(): Promise<string | null> {
  ...
  // projectId comes from app.json (expo.extra.eas.projectId), written by `eas project:init`.
  // In an EAS build it is also resolved automatically when omitted.
  const projectId =
  ...
  return (
    <View style={[styles.tabBar, { paddingBottom: insets.bottom }]}>
      {TABS.map(tab => {
        ...
        const color = isActive ? BRAND_DARK : INACTIVE_GRAY;
        return (
          <Pressable
            key={tab.key}
    ...
    registerForPushNotifications().then(token => {
      if (token) {
        ...
        webViewRef.current?.injectJavaScript(`
          ...
          window.__EXPO_PUSH_PLATFORM__ = ${JSON.stringify(Platform.OS)};
          true;
        `);
      }
    });
    ...
    async (event: WebViewMessageEvent) => {
      ...
      switch (type) {
        ...
        case 'share': {
          try {
            const message =
              Platform.OS === 'ios' ? payload.title : `${payload.title}\n${payload.url}`;
            await Share.share({
              message,
    ...
    return (
      <SafeAreaProvider>
        <SafeAreaView style={styles.errorContainer}>
          ...
          <Image
            source={require('./assets/others/NextRepLogo.png')}
